...
class Lstm(Algorithm):
    ...
    def learning(self,features,dataset,label,kind, epochs=50, patience=None, fraction=1.0):
        ...
        dataset = get_fraction_of_array(dataset, fraction)
        labels = get_fraction_of_array(labels, fraction)
        ...
        if fallback:
            ...
            print("classifier is generated with time step", TIME_STEP)
        #except:
    ...
    def detection(self, dataset, label, kind):
        ...
        r_at_99p = recall_at_Xprecision(np.array(pred).squeeze(), label, 0.99)
        r_at_95p = recall_at_Xprecision(np.array(pred).squeeze(), label, 0.95)
